...
### T11 — Continuity-proof staleness when several claims share an attestation window

**Discovered:** Sprint 5's full-integration stress test (five purchases made within ~90 seconds of each other, all failing near-simultaneously).
**Symptom:** all five `submitClaim` calls reverted inside the Attestcoin precompile with `Continuity proof does not match attestation or checkpoint` — reproduced even after manually refetching a brand-new proof and resubmitting sequentially (no concurrency), ruling out a nonce/RPC race as the sole cause. Five *separately timed* full-flow runs earlier the same session (one claim in flight at a time, minutes apart) all succeeded.
**Analysis:** `docs/ATTESTCOIN_INTEGRATION.md` already notes that a continuity proof's verification cost grows with age as Attestcoin's checkpoint advances — this session's evidence suggests age doesn't just raise cost past some point, it can invalidate the proof outright. Five claims whose `DeliveryFailed` transactions land in the same narrow block range share a continuity proof anchored to the same checkpoint; if that checkpoint rotates before `submitClaim` runs, all of them go stale together, which is consistent with the all-five, uniform failure observed (a real per-claim race would be expected to leave some claims succeeding).
**Impact:** no funds moved and no over-payout occurred — the precompile correctly refused all five rather than verifying incorrectly. The impact is availability, not fund safety: a buyer whose claim's proof goes stale before submission needs a fresh trigger (a new `DeliveryFailed`-equivalent event) to be paid, since `submitClaim`'s anti-replay key is tied to the specific proof.
**Mitigation:** not yet implemented. `cmd/worker` already submits "as soon as attestation is available" per the architecture's stated design, which minimizes this window for an isolated claim; it does not currently serialize or stagger claims whose triggers land close together in time, which real usage could still hit (e.g. many orders created with the same SLA duration around the same time, so their deadlines — and thus their `DeliveryFailed` events — cluster). Candidate fixes for post-hackathon hardening: serialize the wait-for-attestation-through-submit critical section per checkpoint window, or detect this specific revert and retry with a freshly-fetched proof against the *new* checkpoint rather than exhausting retries against the same stale one.
