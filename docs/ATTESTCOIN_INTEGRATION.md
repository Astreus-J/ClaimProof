...
**Update from live testing (Sprints 5 and 7):** proof age turned out to matter beyond gas cost — it can invalidate the proof outright, not just make it more expensive. First seen with several claims sharing a checkpoint window (triggered within seconds of each other); reproduced again with a single, isolated claim simply delayed by a few minutes. Both cases reverted `submitClaim` with `Continuity proof does not match attestation or checkpoint`, even against a freshly-refetched proof — the trigger is elapsed time since attestation, not concurrency. See [THREAT_MODEL.md](THREAT_MODEL.md) T11 for the full writeup — worth knowing for anyone else integrating against this protocol version, since it isn't obvious from the gas-cost documentation alone.
