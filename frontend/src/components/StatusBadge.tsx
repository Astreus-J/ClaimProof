export type BadgeStage = "active" | "failure-detected" | "verifying-proof" | "paid";
...
export default function StatusBadge({ stage }: { stage: BadgeStage }) {
  ...
  return (
    <span
      role="status"
      aria-live="polite"
      className={`inline-flex items-center gap-2 whitespace-nowrap rounded-full border px-3 py-1 text-sm font-medium ${style.classes}`}
    >
      <span
        className={`h-2 w-2 rounded-full ${style.dot} ${stage === "verifying-proof" ? "animate-pulse" : ""}`}
        aria-hidden
      />
      {style.label}
    </span>
