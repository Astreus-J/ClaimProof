...
// Vendored verbatim from the `@gluwa/usc-contracts` npm package (v0.1.2), MIT licensed.
// No installable git repository is published for this package (the `repository` field in
// its package.json points to a 404), so it cannot be added via `forge install`; copied
// here instead of reimplementing Attestcoin's transaction/receipt decoding logic.
// Source: https://www.npmjs.com/package/@gluwa/usc-contracts
